...
CONFORMANCE_PLAN = ROOT / "docs" / "testing" / "rust_native_conformance_plan.md"
...
def main() -> None:
    ...
    if "native backend claims remain blocked until the parity lanes are both implemented and passed as release evidence" not in cert_state:
        errors.append("Phase 4 certification-state report must explicitly keep native backend claims blocked")

    if "differential snapshots between Python and Rust kernel plans" not in conformance_plan:
        errors.append("docs/testing/rust_native_conformance_plan.md must continue to cover kernel parity snapshots")
    if "native backend remains non-claimable until the parity lanes pass as release evidence" not in conformance_plan:
        errors.append("docs/testing/rust_native_conformance_plan.md must keep native claimability fail-closed")

    if "Phase 4 native parity checkpoint" not in implementation_map:
        errors.append("docs/conformance/IMPLEMENTATION_MAP.md must include the Phase 4 native parity checkpoint row")
    ...
    if "validate_phase4_native_parity.py" not in ci_validation:
        errors.append("docs/developer/CI_VALIDATION.md must list the Phase 4 native-parity validator")

    if "Validate Phase 4 native parity" not in workflow:
        errors.append(".github/workflows/policy-governance.yml must run the Phase 4 native-parity validator")

    if "BLK-004" not in blocked_claims or "native backend claims must remain blocked" not in blocked_claims:
        errors.append("certification/claims/blocked.yaml must block native backend claim publication until parity lanes pass")

    fail(errors)
    print("Phase 4 native-parity validation passed")
