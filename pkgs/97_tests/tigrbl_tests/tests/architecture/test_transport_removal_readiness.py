...
TIGRBL_ROOT = PKGS_ROOT / "80_facade" / "tigrbl" / "tigrbl"
CONCRETE_ROOT = PKGS_ROOT / "70_concrete" / "tigrbl_concrete" / "tigrbl_concrete"
TESTS_ROOT = PKGS_ROOT / "97_tests" / "tigrbl_tests" / "tests"
...
def test_deprecated_transport_shims_have_runtime_warnings() -> None:
    for path in DEPRECATED_SHIMS:
        ...
        assert (
            "DeprecationWarning" in text or "warn_deprecated_transport_module" in text
        )
...
def test_jsonrpc_schema_namespace_replaces_transport_models() -> None:
    ...
    public_models = (TIGRBL_ROOT / "transport" / "jsonrpc" / "models.py").read_text(
        encoding="utf-8"
    )
    concrete_models = (CONCRETE_ROOT / "transport" / "jsonrpc" / "models.py").read_text(
        encoding="utf-8"
    )
...
def test_transport_deprecation_has_removal_readiness_coverage() -> None:
    missing = [
        str(path.relative_to(PKGS_ROOT))
