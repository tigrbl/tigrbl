...
def test_from_any_supports_aliases_and_filters_unknown_fields() -> None:
    spec = EngineSessionSpec.from_any({"iso": "serializable", "readonly": True, "x": 1})
...
def test_merge_prefers_non_none_from_higher() -> None:
    base = EngineSessionSpec(isolation="read_committed", read_only=False)
...
def test_session_helpers_and_validation() -> None:
    ...
    assert engine_session_spec(max_retries=3).max_retries == 3

    with pytest.raises(ValueError, match="either a mapping/spec or kwargs"):
        engine_session_spec({"read_only": True}, max_retries=1)
