...
async def _run(obj: object | None, ctx: Any) -> None:
    ...
    if not isinstance(model, type):
        raise TypeError("handler_exists requires a model type")
    setattr(ctx, "result", await _core.exists(model, _ctx.ident(model, ctx), db=_ctx.db(ctx)))


class AtomImpl(Atom[Resolved, Operated, Exception]):
    name = "sys.handler_exists"
    anchor = ANCHOR
