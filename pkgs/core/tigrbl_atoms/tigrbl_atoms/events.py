...
BATCH_TRANSPORT_UNIT_CAPTURE = "batch.transport.unit_capture"
BATCH_TRANSPORT_SINK_BIND = "batch.transport.sink_bind"
BATCH_INTENT_BUILD = "batch.intent.build"
BATCH_PREKEY = "batch.intent.prekey"
BATCH_GROUP_KEY = "batch.intent.group_key"
BATCH_ADMIT = "batch.admit"
BATCH_DEDUPE = "batch.dedupe"
BATCH_SEAL_CHECK = "batch.seal_check"
BATCH_AWAIT_SEAL = "batch.await_seal"
BATCH_TX_BEGIN = "batch.tx.begin"
BATCH_PREPARE_EXECUTE = "batch.prepare_execute"
BATCH_EXECUTE = "batch.execute"
BATCH_RESULT_SLOTS = "batch.result_slots"
BATCH_PRECOMMIT_VALIDATE = "batch.precommit_validate"
BATCH_COMMIT = "batch.commit"
BATCH_EGRESS_SHAPE = "batch.egress.shape"
BATCH_FANOUT_EMIT = "batch.fanout.emit"
BATCH_CLEANUP = "batch.cleanup"
BATCH_REJECT_ADMISSION = "batch.reject_admission"
BATCH_ABORT_GROUP = "batch.abort_group"
BATCH_ERROR_PROJECT = "batch.error_project"
...
_EVENT_ORDER: Tuple[str, ...] = (
    ...
    BATCH_TRANSPORT_UNIT_CAPTURE,
    BATCH_TRANSPORT_SINK_BIND,
...
_ANCHOR_PHASE: Dict[str, Phase] = {
    ...
    BATCH_TRANSPORT_UNIT_CAPTURE: INGRESS_PARSE,
    BATCH_TRANSPORT_SINK_BIND: INGRESS_PARSE,
    ...
    BATCH_INTENT_BUILD: INGRESS_DISPATCH,
    BATCH_PREKEY: INGRESS_DISPATCH,
    DEP_SECURITY: PRE_TX_BEGIN,
    DEP_EXTRA: PRE_TX_BEGIN,
    BATCH_GROUP_KEY: PRE_TX_BEGIN,
    BATCH_ADMIT: PRE_TX_BEGIN,
    BATCH_DEDUPE: PRE_TX_BEGIN,
    BATCH_SEAL_CHECK: PRE_TX_BEGIN,
    BATCH_AWAIT_SEAL: PRE_TX_BEGIN,
    SYS_TX_BEGIN: START_TX,
    BATCH_TX_BEGIN: START_TX,
    SCHEMA_COLLECT_IN: PRE_HANDLER,
    BATCH_PREPARE_EXECUTE: PRE_HANDLER,
    IN_VALIDATE: PRE_HANDLER,
    RESOLVE_VALUES: PRE_HANDLER,
    PRE_FLUSH: PRE_HANDLER,
    EMIT_ALIASES_PRE: PRE_HANDLER,
    HANDLER: HANDLER_PHASE,
    BATCH_EXECUTE: HANDLER_PHASE,
    SYS_HANDLER_PERSISTENCE: HANDLER_PHASE,
    BATCH_RESULT_SLOTS: POST_HANDLER,
    BATCH_PRECOMMIT_VALIDATE: PRE_COMMIT,
    ...
    BATCH_EGRESS_SHAPE: EGRESS_SHAPE,
    BATCH_FANOUT_EMIT: EGRESS_FINALIZE,
    BATCH_CLEANUP: POST_RESPONSE,
    ERR_CTX_BUILD: "ON_ERROR",
    BATCH_REJECT_ADMISSION: "ON_ERROR",
    ERR_CLASSIFY: "ON_ERROR",
    ERR_ROLLBACK: "TX_ROLLBACK",
    BATCH_ABORT_GROUP: "TX_ROLLBACK",
    ERR_TRANSPORT_SHAPE: "ON_ERROR",
    BATCH_ERROR_PROJECT: "ON_ERROR",
}
...
_ANCHOR_STAGE: Dict[str, Tuple[Stage, Stage]] = {
    ...
    BATCH_TRANSPORT_UNIT_CAPTURE: (Ingress, Ingress),
    BATCH_TRANSPORT_SINK_BIND: (Ingress, Ingress),
    ...
    BATCH_INTENT_BUILD: (Bound, Bound),
    BATCH_PREKEY: (Bound, Bound),
    DEP_SECURITY: (Planned, Guarded),
    DEP_EXTRA: (Guarded, Guarded),
    BATCH_GROUP_KEY: (Guarded, Guarded),
    BATCH_ADMIT: (Guarded, Guarded),
    BATCH_DEDUPE: (Guarded, Guarded),
    BATCH_SEAL_CHECK: (Guarded, Guarded),
    BATCH_AWAIT_SEAL: (Guarded, Guarded),
    SYS_TX_BEGIN: (Guarded, Executing),
    BATCH_TX_BEGIN: (Guarded, Executing),
    SCHEMA_COLLECT_IN: (Executing, Executing),
    BATCH_PREPARE_EXECUTE: (Executing, Executing),
    ...
    BATCH_EXECUTE: (Resolved, Operated),
    SYS_HANDLER_PERSISTENCE: (Resolved, Operated),
    BATCH_RESULT_SLOTS: (Operated, Operated),
    BATCH_PRECOMMIT_VALIDATE: (Operated, Operated),
    SYS_TX_COMMIT: (Operated, Operated),
    BATCH_COMMIT: (Operated, Operated),
    ...
    BATCH_EGRESS_SHAPE: (Encoded, Encoded),
    BATCH_FANOUT_EMIT: (Egressed, Egressed),
    BATCH_CLEANUP: (Egressed, Egressed),
    ERR_CTX_BUILD: (Failed, Failed),
    BATCH_REJECT_ADMISSION: (Failed, Failed),
    ERR_CLASSIFY: (Failed, Failed),
    ERR_ROLLBACK: (Failed, Failed),
    BATCH_ABORT_GROUP: (Failed, Failed),
    ERR_TRANSPORT_SHAPE: (Failed, Failed),
    BATCH_ERROR_PROJECT: (Failed, Failed),
}
