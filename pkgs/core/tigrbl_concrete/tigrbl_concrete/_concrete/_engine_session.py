...
class EngineSession(EngineSessionBase):
    """Delegating wrapper for resolved provider engine database sessions."""

    def __init__(self, underlying: Any, spec: Optional[EngineSessionSpec] = None) -> None:
        super().__init__(spec)
        self._u = underlying
    ...
    async def _tx_begin_impl(self) -> None:
        ...
        if not callable(fn):
            raise RuntimeError("underlying engine session does not support begin()")
        rv = fn()
        if inspect.isawaitable(rv):
            await rv
    ...
    async def _tx_commit_impl(self) -> None:
        ...
        if not callable(fn):
            raise RuntimeError("underlying engine session does not support commit()")
        rv = fn()
        if inspect.isawaitable(rv):
            await rv
    ...
    async def _tx_rollback_impl(self) -> None:
        ...
        if not callable(fn):
            raise RuntimeError("underlying engine session does not support rollback()")
        rv = fn()
        if inspect.isawaitable(rv):
            await rv
    ...
    def get_bind(self) -> Any:
        fn = getattr(self._u, "get_bind", None)
        if callable(fn):
            return fn()
        bind = getattr(self._u, "bind", None)
        if bind is not None:
            return bind
        raise RuntimeError("underlying engine session does not expose a bind")

    def _add_impl(self, obj: Any) -> Any:
        fn = getattr(self._u, "add", None)
        if not callable(fn):
            raise NotImplementedError(
                "underlying engine session does not implement add(obj)"
            )
        return fn(obj)
    ...
    async def _delete_impl(self, obj: Any) -> None:
        ...
        if not callable(fn):
            raise NotImplementedError(
                "underlying engine session does not implement delete(obj)"
            )
        rv = fn(obj)
        if inspect.isawaitable(rv):
            await rv
    ...
    async def _get_impl(self, model: type, ident: Any) -> Any | None:
        ...
        if not callable(fn):
            raise NotImplementedError(
                "underlying engine session does not implement get(model, ident)"
            )
        rv = fn(model, ident)
    ...
    async def _execute_impl(self, stmt: Any) -> Any:
        ...
        if not callable(fn):
            raise NotImplementedError(
                "underlying engine session does not implement execute(stmt)"
            )
        rv = fn(stmt)
        return await rv if inspect.isawaitable(rv) else rv
    ...
    async def _executeloop_impl(self, statements: Any) -> Any:
        ...
        if not callable(fn):
            raise NotImplementedError(
                "underlying engine session does not implement executeloop(statements)"
            )
        rv = fn(statements)
        return await rv if inspect.isawaitable(rv) else rv
    ...
    async def _executemany_impl(self, stmt: Any, parameter_sets: Any) -> Any:
        ...
        if not callable(fn):
            raise NotImplementedError(
                "underlying engine session does not implement "
...
def wrap_sessionmaker(
    maker: Callable[..., Any], spec: EngineSessionSpec | None = None
) -> Callable[[], EngineSession]:
    """Wrap a provider session factory to yield EngineSession instances."""

    resolved_spec = spec or EngineSessionSpec()

    def _mk() -> EngineSession:
        underlying = maker()
        session = EngineSession(underlying, resolved_spec)
        session.apply_spec(resolved_spec)
        return session

    return _mk


__all__ = ["EngineSession", "wrap_sessionmaker"]
