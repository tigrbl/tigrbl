...
def _as_mapping(app: Any) -> dict[str, Any]:
    if isinstance(app, str):
        ...
        if not isinstance(payload, dict):
            raise TypeError("native runtime expects a mapping or JSON object")
        return payload
...
def _iter_tables(source: dict[str, Any], app: Any) -> tuple[Any, ...]:
    ...
    if isinstance(raw, Sequence) and not isinstance(raw, (str, bytes, bytearray)):
        return tuple(raw)
    raise TypeError("native runtime expects app.tables to be a mapping or sequence")
...
def _lower_declared_bindings(source: dict[str, Any], app: Any) -> list[dict[str, Any]]:
    ...
    if raw is not None:
        if not (
            ...
            raise TypeError("native runtime expects app.bindings to be a sequence")
...
def _lower_declared_tables(source: dict[str, Any], app: Any) -> list[dict[str, str]]:
    ...
    if raw is not None:
        if not (
            ...
            raise TypeError("native runtime expects app.tables to be a sequence")
        if raw and all(isinstance(item, Mapping) for item in raw):
            return [dict(item) for item in raw]
...
def _lower_declared_engines(source: dict[str, Any], app: Any) -> list[dict[str, Any]]:
    ...
    if raw is not None:
        if not (
            ...
            raise TypeError("native runtime expects app.engines to be a sequence")
        if raw:
            return [
                dict(item)
...
def _lower_callbacks(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not (
        ...
        raise TypeError("native runtime expects app.callbacks to be a sequence")
    payload: list[dict[str, Any]] = []
    for item in raw:
        if isinstance(item, Mapping):
...
def build_native_app_spec(app: Any) -> dict[str, Any]:
    ...
    if not payload["name"]:
        raise ValueError("native runtime requires a non-empty app name")
    return payload


def coerce_native_spec_dict(app: Any) -> dict[str, Any]:
    if isinstance(app, str):
        payload = json.loads(app)
        if not isinstance(payload, dict):
            raise TypeError("native runtime expects a mapping or JSON object")
        return payload
    if isinstance(app, Mapping):
        return build_native_app_spec(dict(app))
    return build_native_app_spec(app)


def coerce_native_spec_json(app: Any) -> str:
    return json.dumps(coerce_native_spec_dict(app), sort_keys=True)
