...
TMP_DIR = ROOT / ".tmp"
RESULTS_PATH = PERF_DIR / "benchmark_results_streaming_uvicorn.json"
KERNEL_JSON_PATH = PERF_DIR / "kernel-plan-benchmark-streaming.json"
KERNEL_MD_PATH = PERF_DIR / "kernel-plan-benchmark-streaming.md"
TIGRBL_CALL_GRAPH_PATH = PERF_DIR / "tigrbl_streaming_call_graph_250_ops.json"
FASTAPI_CALL_GRAPH_PATH = PERF_DIR / "fastapi_streaming_call_graph_250_ops.json"
HOT_BLOCK_PREFIX = PERF_DIR / "tgpkhot1-benchmark-streaming"
OPS_COUNTS = (25, 250)
WARMUP_OPS = 5
...
async def _benchmark_stream_app(
    *,
...
async def _export_hot_block() -> list[str]:
    with TemporaryDirectory(dir=TMP_DIR, ignore_cleanup_errors=True) as tmp_dir:
        db_path = Path(tmp_dir) / "streaming-hot-block.sqlite3"
        app = create_tigrbl_streaming_db_app(db_path)
        plan = build_kernel_plan(app)
        packed = getattr(plan, "packed", None)
        if packed is None:
            raise RuntimeError("streaming benchmark app did not build a packed kernel")
        hot_block_bytes = getattr(packed, "hot_block_bytes", b"")
        if not hot_block_bytes:
            raise RuntimeError("streaming benchmark app did not carry TGPKHOT1 bytes")
        ...
        outputs = {
            ...
            HOT_BLOCK_PREFIX.with_suffix(".benchmark.md"): _render_hot_block_benchmark_md("streaming", benchmark_payload).encode("utf-8"),
        }
...
async def _profile_streaming_call_graph(
    *,
    create_app: Callable[[Path], Any],
    endpoint: str,
    expected_body: bytes,
...
async def main() -> None:
    ...
    transport_body = expected_transport_stream_bytes()
    db_body = expected_db_stream_bytes()

    results: dict[str, Any] = {"transport_only": {}, "db_backed": {}}
    for ops in OPS_COUNTS:
        results["transport_only"][str(ops)] = {
            "tigrbl": await _benchmark_stream_app(
                create_app=create_tigrbl_streaming_transport_app,
                scenario=f"tigrbl-stream-transport-{ops}",
                endpoint="/stream/fixed",
                expected_body=transport_body,
                ops=ops,
            ),
            "fastapi": await _benchmark_stream_app(
                create_app=create_fastapi_streaming_transport_app,
                scenario=f"fastapi-stream-transport-{ops}",
                endpoint="/stream/fixed",
                expected_body=transport_body,
                ops=ops,
            ),
        ...
        results["db_backed"][str(ops)] = {
            "tigrbl": await _benchmark_stream_app(
                create_app=create_tigrbl_streaming_db_app,
                scenario=f"tigrbl-stream-db-{ops}",
                endpoint="/stream/items",
                expected_body=db_body,
                ops=ops,
            ),
            "fastapi": await _benchmark_stream_app(
                create_app=create_fastapi_streaming_db_app,
                scenario=f"fastapi-stream-db-{ops}",
                endpoint="/stream/items",
                expected_body=db_body,
                ops=ops,
            ),
    ...
    with TemporaryDirectory(dir=TMP_DIR, ignore_cleanup_errors=True) as tmp_dir:
        db_path = Path(tmp_dir) / "streaming-kernel.sqlite3"
        app = create_tigrbl_streaming_db_app(db_path)
    ...
    kernel_payload = {
        "fairness": {
            ...
            "transport_only_endpoint": "/stream/fixed",
            "db_backed_endpoint": "/stream/items",
        },
        "candidate": {
    ...
    await _profile_streaming_call_graph(
        create_app=create_tigrbl_streaming_db_app,
        endpoint="/stream/items",
        expected_body=db_body,
