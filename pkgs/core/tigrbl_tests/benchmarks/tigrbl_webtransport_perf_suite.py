...
from tests.perf.helper_webtransport_apps import (
    create_tigrbl_webtransport_db_app,
    create_tigrbl_webtransport_transport_app,
    fetch_webtransport_names,
)

ROOT = Path(__file__).resolve().parents[4]
PERF_DIR = ROOT / "pkgs" / "core" / "tigrbl_tests" / "tests" / "perf"
TMP_DIR = ROOT / ".tmp"
RESULTS_PATH = PERF_DIR / "benchmark_results_webtransport_uvicorn.json"
KERNEL_JSON_PATH = PERF_DIR / "kernel-plan-benchmark-webtransport.json"
KERNEL_MD_PATH = PERF_DIR / "kernel-plan-benchmark-webtransport.md"
TIGRBL_CALL_GRAPH_PATH = PERF_DIR / "tigrbl_webtransport_call_graph_250_ops.json"
TIGRBL_TRANSPORT_CALL_GRAPH_PATH = (
    PERF_DIR / "tigrbl_webtransport_transport_call_graph_250_ops.json"
)
HOT_BLOCK_PREFIX = PERF_DIR / "tgpkhot1-benchmark-webtransport"
OPS_COUNTS = (25, 250)
WARMUP_OPS = 5
...
async def _run_webtransport_session(
    ...
    *,
    ...
    pending = [
        ...
        {"type": "webtransport.receive", "text": text},
    ]
    ...
    async def receive() -> dict[str, Any]:
        return (
            pending.pop(0)
            if pending
            else {"type": "webtransport.disconnect", "code": 0}
        )
...
def _extract_text_payload(sent: list[dict[str, Any]]) -> str:
    for message in sent:
        if message.get("type") != "webtransport.send":
            continue
    ...
    raise AssertionError(f"no webtransport.send payload found in {sent!r}")


async def _benchmark_webtransport_app(
    *,
    ...
    with TemporaryDirectory(dir=TMP_DIR, ignore_cleanup_errors=True) as tmp_dir:
        ...
        if db_backed:
            for idx in range(WARMUP_OPS):
                name = f"warmup-wt-item-{idx}"
                sent = await _run_webtransport_session(
                    app,
                    path=endpoint,
            ...
            names = fetch_webtransport_names(db_path)
            assert names[-ops:] == persisted_names
        else:
            for idx in range(WARMUP_OPS):
                text = f"warmup-wt-echo-{idx}"
                sent = await _run_webtransport_session(app, path=endpoint, text=text)
                assert _extract_text_payload(sent) == text
            start = perf_counter()
            for idx in range(ops):
                text = f"bench-wt-echo-{idx}"
                sent = await _run_webtransport_session(app, path=endpoint, text=text)
                assert _extract_text_payload(sent) == text
            elapsed = perf_counter() - start
        return {
            ...
            "shared_runner": "direct ASGI webtransport session",
        }
...
async def _export_hot_block() -> list[str]:
    with TemporaryDirectory(dir=TMP_DIR, ignore_cleanup_errors=True) as tmp_dir:
        db_path = Path(tmp_dir) / "webtransport-hot-block.sqlite3"
        app = create_tigrbl_webtransport_db_app(db_path)
        plan = build_kernel_plan(app)
        packed = getattr(plan, "packed", None)
        if packed is None:
            raise RuntimeError("webtransport benchmark app did not build a packed kernel")
        hot_block_bytes = getattr(packed, "hot_block_bytes", b"")
        if not hot_block_bytes:
            raise RuntimeError("webtransport benchmark app did not carry TGPKHOT1 bytes")
        ...
        outputs = {
            ...
            HOT_BLOCK_PREFIX.with_suffix(".benchmark.md"): _render_hot_block_benchmark_md("webtransport", benchmark_payload).encode("utf-8"),
        }
...
async def _profile_webtransport_call_graph(
    *,
    create_app: Callable[[Path], Any],
    endpoint: str,
    db_backed: bool,
...
async def main() -> None:
    ...
    for ops in OPS_COUNTS:
        results["transport_only"][str(ops)] = {
            "tigrbl": await _benchmark_webtransport_app(
                create_app=create_tigrbl_webtransport_transport_app,
                scenario=f"tigrbl-webtransport-transport-{ops}",
        ...
        results["db_backed"][str(ops)] = {
            "tigrbl": await _benchmark_webtransport_app(
                create_app=create_tigrbl_webtransport_db_app,
                scenario=f"tigrbl-webtransport-db-{ops}",
                endpoint="/transport/items",
                ops=ops,
                db_backed=True,
            )
        }
    ...
    with TemporaryDirectory(dir=TMP_DIR, ignore_cleanup_errors=True) as tmp_dir:
        db_path = Path(tmp_dir) / "webtransport-kernel.sqlite3"
        app = create_tigrbl_webtransport_db_app(db_path)
        measurement = _serialize_measurement(app)
    kernel_payload = {
        "fairness": {
            "surface": "webtransport",
            "shared_server_runner": "direct ASGI webtransport session",
            "shared_database_family": "SQLite",
            "transport_only_endpoint": "/transport/echo",
            "db_backed_endpoint": "/transport/items",
            "parity": "tigrbl_only",
        },
        "candidate": {
    ...
    KERNEL_MD_PATH.write_text(
        "# WebTransport Kernel Packing KC Report\n\n"
        f"- shared runner: direct ASGI webtransport session\n"
        f"- raw bytes: {measurement['raw_bytes']}\n"
        f"- compressed bytes: {measurement['compressed_bytes']}\n"
        f"- transport-only 250 ops/s: tigrbl={results['transport_only']['250']['tigrbl']['ops_per_second']:.2f}\n"
        f"- db-backed 250 ops/s: tigrbl={results['db_backed']['250']['tigrbl']['ops_per_second']:.2f}\n",
        encoding="utf-8",
    )
    RESULTS_PATH.write_text(json.dumps(results, indent=2), encoding="utf-8")
    await _profile_webtransport_call_graph(
        create_app=create_tigrbl_webtransport_transport_app,
        endpoint="/transport/echo",
        db_backed=False,
