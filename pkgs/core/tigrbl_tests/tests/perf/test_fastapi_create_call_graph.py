...
TOP_EDGE_LIMIT = 150
RESULTS_PATH = Path(__file__).with_name("fastapi_create_call_graph_250_ops.json")
...
async def _profile_create_call_graph(*, results_path: Path) -> dict[str, Any]:
    ...
    with TemporaryDirectory(dir=PERF_TEMP_ROOT, ignore_cleanup_errors=True) as tmp_dir:
        ...
        db_path = base_dir / "fastapi-create-call-graph.sqlite3"
...
@pytest.mark.perf
def test_fastapi_create_call_graph_250_ops() -> None:
