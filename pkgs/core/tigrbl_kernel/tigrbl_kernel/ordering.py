...
_PREF: Dict[str, Tuple[str, ...]] = {
    ...
    _ev.INGRESS_TRANSPORT_EXTRACT: (
        "ingress:transport_extract",
        "transport:unit_capture",
        "transport:sink_bind",
    ),
    _ev.INGRESS_INPUT_PREPARE: (
    ...
    _ev.SCHEMA_COLLECT_IN: ("schema:collect_in", "batch:prepare_execute"),
    _ev.IN_VALIDATE: ("wire:build_in", "wire:validate_in"),
    _ev.SYS_TX_BEGIN: ("sys:start_tx", "batch:tx_begin"),
    ...
    _ev.SYS_HANDLER_PERSISTENCE: ("batch:execute", "sys:handler_persistence"),
    _ev.SYS_TX_COMMIT: (
        "batch:precommit_validate",
        "sys:commit_tx",
        "batch:commit",
    ),
    _ev.POST_FLUSH: ("refresh:demand", "batch:result_slots"),
    _ev.EMIT_ALIASES_POST: ("emit:paired_post",),
    _ev.SCHEMA_COLLECT_OUT: ("schema:collect_out",),
    _ev.OUT_BUILD: ("wire:build_out",),
    _ev.EGRESS_RESULT_NORMALIZE: ("egress:result_normalize", "fanout:shape"),
    ...
    _ev.EGRESS_ASGI_SEND: ("egress:asgi_send", "fanout:emit_many"),
