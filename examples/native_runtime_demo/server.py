...
from app_spec import build_native_payload, compose_app_spec
...
NATIVE_PAYLOAD = build_native_payload()
RUNTIME = Runtime(executor_backend="rust")
NATIVE_HANDLE = RUNTIME.native_handle(APP_SPEC)


class NativeRuntimeDemoHandler(BaseHTTPRequestHandler):
    server_version = "tigrbl-native-runtime-demo/0.2"
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/healthz":
            self._write_json(
                HTTPStatus.OK,
                {
                    ...
                    "executor": "rust",
                    "runtime": NATIVE_HANDLE.describe(),
                },
            )
            return
        ...
        if match:
            response = NATIVE_HANDLE.execute_rest(
                {
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path == "/rpc":
            ...
            response = NATIVE_HANDLE.execute_jsonrpc(envelope)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Serve a Python-authored Tigrbl AppSpec via the Rust-native runtime."
    )
...
def main() -> None:
    args = parse_args()
    server = HTTPServer((args.host, args.port), NativeRuntimeDemoHandler)
    print(
        json.dumps(
            {
                "host": args.host,
