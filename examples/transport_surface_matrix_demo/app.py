...
def _demo_db_path() -> Path:
    configured = os.environ.get("TIGRBL_TRANSPORT_SURFACE_MATRIX_DEMO_DB")
    if configured:
        return Path(configured)
    return Path(__file__).with_name("transport_surface_matrix_demo.sqlite3")
...
def build_app(db_path: str | Path | None = None) -> TigrblApp:
    ...
    app = TigrblApp(
        title="Tigrbl Transport Surface Matrix Demo",
        version="0.1.0",
        description="Repo-owned matrix demo for REST, JSON-RPC, stream, SSE, websocket, mTLS, and WebTransport transport surfaces.",
