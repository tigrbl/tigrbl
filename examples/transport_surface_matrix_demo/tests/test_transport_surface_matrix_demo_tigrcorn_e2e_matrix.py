...
APP_PATH = EXAMPLE_ROOT / "app.py"
_TEMP_DEMO_DBS: list[tempfile.TemporaryDirectory[str]] = []


def _load_transport_surface_matrix_demo_app_module(
    tempdir: tempfile.TemporaryDirectory[str],
) -> Any:
    spec = importlib.util.spec_from_file_location("transport_surface_matrix_demo_app", APP_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load transport surface matrix demo app from {APP_PATH}")
    module = importlib.util.module_from_spec(spec)
    default_db = Path(tempdir.name) / "transport_surface_matrix_demo_default.sqlite3"
    old_db = os.environ.get("TIGRBL_TRANSPORT_SURFACE_MATRIX_DEMO_DB")
    os.environ["TIGRBL_TRANSPORT_SURFACE_MATRIX_DEMO_DB"] = str(default_db)
    try:
        spec.loader.exec_module(module)
    finally:
        if old_db is None:
            os.environ.pop("TIGRBL_TRANSPORT_SURFACE_MATRIX_DEMO_DB", None)
        else:
            os.environ["TIGRBL_TRANSPORT_SURFACE_MATRIX_DEMO_DB"] = old_db
    return module


def build_app() -> Any:
    tempdir = tempfile.TemporaryDirectory(
        prefix="tigrbl-transport-surface-matrix-demo-",
        ignore_cleanup_errors=True,
    )
    _TEMP_DEMO_DBS.append(tempdir)
    module = _load_transport_surface_matrix_demo_app_module(tempdir)
    return module.build_app(db_path=Path(tempdir.name) / "transport_surface_matrix_demo.sqlite3")
...
@pytest.mark.asyncio
async def test_transport_surface_matrix_demo_tigrcorn_e2e_matrix() -> None:
...
@pytest.mark.asyncio
async def test_transport_surface_matrix_demo_t2_negative_http_and_mtls_cases() -> None:
